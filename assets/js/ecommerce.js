...
async function saveProductsToLocalStorage(){
    const items = await fetchDummyJson('products?limit=100');
    const productsLength = items.products
    localStorage.setItem('products', JSON.stringify(productsLength));
}


async function loadMoreProducts() {
    const items = await fetchDummyJson(`products?limit=9&skip=${skip}`);
    const products = items.products;
